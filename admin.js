...
module.exports=function(){
    ...
    router.post('/update-user',function(req,res){
        var UserInfo={
            ...
            //othermsg
            id:req.body.user_id
//{"porject":"三门","department":"HSE部","sex":"male","othermsg":"","user_id":"4"}
        };

        var updateStr='UPDATE user_table SET porject="'+ UserInfo.porject +'", department="'+ UserInfo.department +' " ,sex="'+UserInfo.sex+'" WHERE id="'+UserInfo.id+'"';
        console.log(updateStr);//测试数据 可以删
        connection.query(updateStr,(err,data)=>{
            if(err){
                res.send(err).end();
            }else{
                common.parseCommand(data);
                res.redirect('/user');
            }
        });
    })
